...
bool TopDownSyntaxAnalyzer::isStatement() {
    Node * parent = startNonTerminal("<Statement> -> <Assign> | <Declaration> | <If> | <While> | <Conditional>"); // I put <Conditional> in Statement as a test. May need to move it to follow <If>
    ...
    // if (isOperator(*nextToken) && isOperatorTopDown())
    // {
    //     finishNonTerminal(parent);
    //     return true;
    // }
...
bool TopDownSyntaxAnalyzer::isIfTopDown() {
  print("<If> -> if <Conditional>");
  Record * record = getNextToken();
  Node * parent = startNonTerminal("<If> -> if <Conditional>");
  if (record == nullptr) {return false;}
  if (isIf(*record)) {
        if (isConditionalTopDown())
        {
...
{
    Node * parent = startNonTerminal("<Conditional> -> (<Expression> <Operator> <Expression>) | (<Expression>) | <Bool>");
    Record *record = getNextToken();
    if (record == nullptr)
        return false;
    // if (record->lexeme == "(")
    ...
    //currentNode->add(new Node(*record));
        if (isE()) 
        {
            print("<Conditional> -> <Expression> <Operator> <Expression>");
            finishNonTerminal(parent);
            return true;
        }
    //}
...
// bool TopDownSyntaxAnalyzer::isOperatorTopDown() {
//   Record * record = getNextToken();
//   Node * parent = startNonTerminal("<Operator> -> < | > | % | <= | >= | <>");
//   if (record == nullptr)
//     return false;
...
bool TopDownSyntaxAnalyzer::isQ(){
    ...
    if (record->lexeme == "+") {
        ...
        if (isT()) {
            if (isQ()) {
                print ("<ExpressionPrime> -> [THIS IS A CONDITIONAL OPERATOR] <Term><ExpressionPrime>");
                finishNonTerminal(parent);
                return true;
            }
        }    
...
bool TopDownSyntaxAnalyzer::isR() {
    ...
    } else if (record->lexeme == "+" || record->lexeme == "-" || record->lexeme == ")" || record->lexeme == ";" || record->lexeme == "<") {
        //cout << *record << endl;
